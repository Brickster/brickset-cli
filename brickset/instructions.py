...
def _parse_pdf_number(instruction_url: str) -> str | None:
...
def _clean_region(region_to_clean: str) -> str:
...
def _region_book(set_number: str, pdf_number: str | None, match: re.Match[str], _description: str) -> str:
...
def _book_region(set_number: str, pdf_number: str | None, match: re.Match[str], _description: str) -> str:
...
def _book_only(set_number: str, pdf_number: str | None, match: re.Match[str], _description: str) -> str:
    return f'{set_number}_b{match.group(1)}_{pdf_number}.pdf'


@_rule(r'^BI.*?\s+(IN|NA|V/?[\.\s]*\d+(?:((?:/|\s[+&]\s)V?\d+)+)?)$')
def _region_only(set_number: str, pdf_number: str | None, match: re.Match[str], description: str) -> str:
    region = _clean_region(match.group(1))
...
def _book_of_books(set_number: str, pdf_number: str | None, match: re.Match[str], _description: str) -> str:
    return f'{set_number}_b{match.group(1)}_{pdf_number}.pdf'


@_rule(r'^\d+_\d+_build_main$', re.IGNORECASE)
def _build_main(set_number: str, pdf_number: str | None, _match: re.Match[str], _description: str) -> str:
    return f'{set_number}_{pdf_number}.pdf'


@_rule(r'^\d+_(\d+)_(?:BI_)?Build_Alt$', re.IGNORECASE)
def _build_alt(set_number: str, _pdf_number: str | None, match: re.Match[str], _description: str) -> str:
    return f'{set_number}_alt_{match.group(1)}.pdf'


@_rule(r'\d+_[a-z]+', re.IGNORECASE)
def _set_short_desc(set_number: str, _pdf_number: str | None, _match: re.Match[str], description: str) -> str:
...
def _inspirational_material_region(set_number: str, pdf_number: str | None, match: re.Match[str], _description: str) -> str:
    region = _clean_region(match.group(1))
    return f'{set_number}_{region}_{pdf_number}.pdf'


@_rule(r'^INSPIRATIONAL MATERIAL,', re.IGNORECASE)
def _inspirational_material(set_number: str, pdf_number: str | None, _match: re.Match[str], _description: str) -> str:
    return f'{set_number}_{pdf_number}.pdf'


@_rule(r'^(?:BI|BUI?LDING\s*INSTRUCTION).*?[\s-]+(\d+)$')
def _other(set_number: str, pdf_number: str | None, _match: re.Match[str], _description: str) -> str:
    return f'{set_number}_{pdf_number}.pdf'


def _construct_instruction_filename(set_number: str, instruction_description: str, instruction_url: str) -> str | None:
...
def download_instruction(directory: str, set_number: str, instruction: dict[str, str]) -> None:
    url = instruction['URL']
